...
@onready var shapecast_radius = shapecast.shape.radius;
...
func is_group_member_nearby(group_name: StringName, desired_dist: float = shapecast_radius) -> bool:
...
func get_object_around(group_name: StringName, desired_dist: float = shapecast_radius) -> Node3D:
	var nodes = get_tree().get_nodes_in_group(group_name)
	for i in nodes.size():
