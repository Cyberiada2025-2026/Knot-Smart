...
func use_rope(raycast_result: Dictionary) -> void:
	...
	if active_rope:
		active_rope.change_point(
			...
			ControlStrategyUtilities.create_marker(raycast_result.collider, raycast_result.position, sphere)
		)
		active_rope = null

	else:
		var target_marker = ControlStrategyUtilities.create_marker_from_unsafe_raycast(raycast_result, sphere)
		
		var player = get_node("../..")
		var player_marker = ControlStrategyUtilities.create_marker_on_player(player, sphere)
		
		active_rope = ControlStrategyUtilities.create_rope(rope_params, [raycast_result.collider, player], [target_marker, player_marker])
