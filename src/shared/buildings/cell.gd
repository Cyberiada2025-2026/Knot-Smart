...
func split(gen_params: RoomGenerationParams) -> Array[Cell]:
	...
	match direction:
		Direction.X:
			var split_point = randi_range(gen_params.min_room_size.x, self.size_x()-gen_params.min_room_size.x)
			e1 = Vector3i(self.start.x + split_point, self.end.y, self.end.z)
			s2 = Vector3i(self.start.x + split_point, self.start.y, self.start.z)
		Direction.Y:
			var split_point = randi_range(gen_params.min_room_size.y, self.size_y()-gen_params.min_room_size.y)
			e1 = Vector3i(self.end.x, self.start.y + split_point, self.end.z)
			s2 = Vector3i(self.start.x, self.start.y + split_point, self.start.z)
		Direction.Z:
			var split_point = randi_range(gen_params.min_room_size.z, self.size_z()-gen_params.min_room_size.z)
...
func get_split_direction(gen_params: RoomGenerationParams) -> Cell.Direction:
	...
	if (size_y() > gen_params.min_room_size.y and y_split_chance != 0) or (self.size_x() <= gen_params.min_room_size.x && self.size_z() <= gen_params.min_room_size.z):
		return Direction.Y
	...
	var randomizer = randi_range(-gen_params.room_split_direction_randomizer, gen_params.room_split_direction_randomizer)
...
func center() -> Vector3:
	return Vector3(self.start.x + float(self.size_x())/2, self.start.y + float(self.size_y())/2, self.start.z + float(self.size_z())/2)
...
func check_overlap_xy(other: Cell) -> BorderInfo:
	...
	info.is_overlapping = overlaps(self.start.x, self.end.x, other.start.x, other.end.x) and overlaps(self.start.y, self.end.y, other.start.y, other.end.y)
	info.overlap_start = Vector3i(maxi(self.start.x, other.start.x), maxi(self.start.y, other.start.y), 0)
...
func check_overlap_xz(other: Cell) -> BorderInfo:
	var info = BorderInfo.new()
	info.is_overlapping = overlaps(self.start.x, self.end.x, other.start.x, other.end.x) and overlaps(self.start.z, self.end.z, other.start.z, other.end.z)
	info.overlap_start = Vector3i(maxi(self.start.x, other.start.x), 0, maxi(self.start.z, other.start.z))
...
func check_overlap_yz(other: Cell) -> BorderInfo:
	var info = BorderInfo.new()
	info.is_overlapping = overlaps(self.start.y, self.end.y, other.start.y, other.end.y) and overlaps(self.start.z, self.end.z, other.start.z, other.end.z)
	info.overlap_start = Vector3i(0, maxi(self.start.y, other.start.y), maxi(self.start.z, other.start.z))
